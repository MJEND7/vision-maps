...
export function SearchVisionNodes({
    ...
    const handleFilterSelect = useCallback((variant: string) => {
    ...
    return (
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    className="fixed z-[50] inset-0 flex items-center justify-center bg-black/10"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                ...
                >
                    ...
                    >
                        {/* 🔍 Search Bar */}
                        <div className="px-4 relative flex gap-2 items-center py-1.5 h-12">
                                ...
                                >
                                    <div className="flex flex-col gap-1">
                                        <button
                                            ...
                                            className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-colors ${selectedVariant === ""
                                        ...
                                        {NODE_VARIANTS.map((variant) => (
                                            <button
                                                key={variant.value}
                                                onClick={() => handleFilterSelect(variant.value)}
                                                className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-colors ${selectedVariant === variant.value
                                                        ? "bg-primary text-primary-foreground"
                                                        : "hover:bg-accent text-foreground"
                                                    }`}
                                            >
