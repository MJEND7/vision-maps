...
export function TranscriptionNodeContent({
    ...
    // array of refs, one for each transcript chunk
    ...
    // Highlight + scroll active transcript chunk
    useEffect(() => {
        ...
        if (newIndex !== activeChunkIndex) {
            ...
            const el = chunkRefs.current[newIndex];
    ...
    const togglePlayPause = () => wavesurfer?.playPause();
    ...
    return (
        <div className="w-full bg-card border border-border rounded-xl p-5 space-y-5 shadow-md">
            ...
            {/* Transcript */}
            <div
                key={`transcript-${audioUrl}`}
                ref={scrollContainerRef}
                className="max-h-[220px] overflow-y-auto overscroll-contain no-scrollbar border-t border-border/50 pt-4 space-y-2"
            >
